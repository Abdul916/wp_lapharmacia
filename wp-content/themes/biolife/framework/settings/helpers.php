 ...
 * GET POST TYPE
 **/
if ( !function_exists( 'biolife_get_post_type' ) ) {
    function biolife_get_post_type()
    {
...
/**
 * Check edit by gutenberg
 */
if ( !function_exists( 'biolife_edited_by_gutenberg' ) ) {
    function biolife_edited_by_gutenberg()
    {
        if ( is_admin() || is_customize_preview() ) {
            return false;
        }

        if ( biolife_get_option( 'disable_gutenberg_scripts' ) == true ) {
            return true;
        }
 ...
 * EFFECT STYLE
 **/
if ( !function_exists( 'biolife_nav_style' ) ) {
    function biolife_effect_style( $unset = array() )
    {
        $effect = array(
            ''                              => esc_html__( 'None', 'biolife' ),
            'effect normal-effect'          => esc_html__( 'Normal Effect', 'biolife' ),
            'effect normal-effect dark-bg'  => esc_html__( 'Normal Effect Dark', 'biolife' ),
            'effect effect faded-in'        => esc_html__( 'Faded In', 'biolife' ),
            'effect bounce-in'              => esc_html__( 'Bounce In', 'biolife' ),
            'effect gray-filter'            => esc_html__( 'Gray Filter', 'biolife' ),
            'effect background-zoom'        => esc_html__( 'Background Zoom', 'biolife' ),
            'effect background-slide'       => esc_html__( 'Background Slide', 'biolife' ),
            'effect rotate-in rotate-left'  => esc_html__( 'Rotate Left In', 'biolife' ),
            'effect rotate-in rotate-right' => esc_html__( 'Rotate Right In', 'biolife' ),
            'effect plus-zoom'              => esc_html__( 'Plus Zoom', 'biolife' ),
            'effect border-zoom'            => esc_html__( 'Border Zoom', 'biolife' ),
            'effect border-scale'           => esc_html__( 'Border ScaleUp', 'biolife' ),
            'effect border-plus'            => esc_html__( 'Border Plus', 'biolife' ),
            'effect overlay-plus'           => esc_html__( 'Overlay Plus', 'biolife' ),
            'effect overlay-cross'          => esc_html__( 'Overlay Cross', 'biolife' ),
            'effect overlay-horizontal'     => esc_html__( 'Overlay Horizontal', 'biolife' ),
            'effect overlay-vertical'       => esc_html__( 'Overlay Vertical', 'biolife' ),
            'effect flashlight'             => esc_html__( 'Flashlight', 'biolife' ),
        );
        if ( !empty( $unset ) ) {
 ...
 **/
if ( !function_exists( 'biolife_nav_style' ) ) {
    function biolife_nav_style()
    {
        return array(
            ''       => esc_html__( 'Nav 01', 'biolife' ),
            'nav-02' => esc_html__( 'Nav 02', 'biolife' ),
            'nav-03' => esc_html__( 'Nav 03', 'biolife' ),
            'nav-04' => esc_html__( 'Nav 04', 'biolife' ),
            'nav-05' => esc_html__( 'Nav 05', 'biolife' ),
        );
    }
}
if ( !function_exists( 'biolife_dot_style' ) ) {
    function biolife_dot_style()
    {
        return array(
            ''       => esc_html__( 'Dot 01', 'biolife' ),
            'dot-02' => esc_html__( 'Dot 02', 'biolife' ),
        );
    }
}
/**
...
if ( !function_exists( 'biolife_get_post_views' ) ) {
    function biolife_get_post_views( $postID = false, $count_key = 'ovic_post_views_count' )
    {
        if ( !$postID ) {
            $postID = get_the_ID();
        }
 ...
 **/
if ( !function_exists( 'biolife_is_mobile' ) ) {
    function biolife_is_mobile()
    {
        $mobile        = false;
        $mobile_enable = biolife_get_option( 'mobile_enable' );
 ...
 * GET OPTION
 **/
if ( !function_exists( 'biolife_get_option' ) ) {
    function biolife_get_option( $option_name = '', $default = '' )
    {
        ...
        $options = apply_filters( 'biolife_get_customize_option', $cs_option, $option_name, $default );
...
if ( !function_exists( 'biolife_preview_options' ) ) {
    function biolife_preview_options( $name )
    {
        $cache_key       = '';
        ...
        if ( biolife_get_option( 'enable_cache_option' ) ) {
            $cache_key = sanitize_key( implode( '-', array( 'biolife-preview', $name, BIOLIFE ) ) );
...
if ( !function_exists( 'biolife_get_header' ) ) {
    function biolife_get_header()
    {
        $header_options = biolife_theme_option_meta(
...
if ( !function_exists( 'biolife_get_footer' ) ) {
    function biolife_get_footer()
    {
        $footer_option = biolife_theme_option_meta(
            '_custom_metabox_theme_options',
            'footer_template',
            'metabox_footer_template',
            'style-01'
        );
        $mobile_footer = biolife_get_option( 'mobile_footer', 'inherit' );
        if ( biolife_is_mobile() && $mobile_footer != 'inherit' ) {
            $footer_option = biolife_get_option( 'mobile_footer', 'inherit' );
        }
 ...
 */
if ( !function_exists( 'biolife_edit_link' ) ) {
    function biolife_edit_link( $id = 0, $text = null )
    {
        if ( !$post = get_post( $id ) ) {
            return;
        }
        ...
        if ( null === $text ) {
            $text = esc_html__( 'Edit This', 'biolife' );
        }
        /**
...
if ( !function_exists( 'biolife_theme_option_meta' ) ) {
    function biolife_theme_option_meta( $meta_id, $option_key, $key_meta = '', $default = '' )
    {
        $ID = get_the_ID();
        ...
        if ( $option_key == null ) {
            ...
            $theme_options = biolife_get_option( $option_key, $default );
        }
...
if ( !function_exists( 'biolife_get_logo' ) ) {
    function biolife_get_logo()
    {
        ...
        $logo        = biolife_theme_option_meta( '_custom_metabox_theme_options', 'logo' );
        $logo_mobile = biolife_get_option( 'logo_mobile' );
        $text        = apply_filters( 'biolife_logo_text_1', esc_html__( 'Biolife', 'biolife' ) );
        if ( biolife_is_mobile() && !empty( $logo_mobile ) ) {
            $logo = $logo_mobile;
        }
        $html = '<' . $tag . ' class="logo">';
        if ( $logo != '' ) {
            ...
            $logo_text = '<span class="icon main-icon-logo_icon"></span><span class="text">' . $text . '</span>';
            $html      .= '<a class="logo-text" href="' . esc_url( $logo_link ) . '">' . $logo_text . '</a>';
        }
        $html .= '</' . $tag . '>';

        return apply_filters( 'biolife_site_logo', $html );
    }
}
/**
 ...
 *
 */
if ( !function_exists( 'biolife_do_shortcode' ) ) {
    function biolife_do_shortcode( $tag, array $atts = array(), $content = null )
    {
...
if ( !function_exists( 'biolife_locate_template' ) ) {
    function biolife_locate_template( $template_name, $template_path = '', $default_path = '' )
    {
        if ( !$template_path ) {
        ...
        return apply_filters( 'biolife_locate_template', $template, $template_name, $template_path );
    }
}
if ( !function_exists( 'biolife_get_template' ) ) {
    function biolife_get_template( $template_name, $args = array(), $template_path = '', $default_path = '' )
    {
        ...
        if ( !$template ) {
            $template = biolife_locate_template( $template_name, $template_path, $default_path );
            wp_cache_set( $cache_key, $template, 'ovic' );
        }

        // Allow 3rd party plugin filter template file from their plugin.
        $filter_template = apply_filters( 'biolife_get_template', $template, $template_name, $args, $template_path, $default_path );
        ...
        if ( $filter_template !== $template ) {
            if ( !file_exists( $filter_template ) ) {
                ...
                _doing_it_wrong(
                    __FUNCTION__,
                    sprintf( __( '%s does not exist.', 'biolife' ), '<code>' . $template . '</code>' ),
                    '2.1'
                );
        ...
        if ( !empty( $args ) && is_array( $args ) ) {
            if ( isset( $args['action_args'] ) ) {
                _doing_it_wrong(
                    __FUNCTION__,
                    __( 'action_args should not be overwritten when calling biolife_get_template.', 'biolife' ),
                    '3.6.0'
                );
        ...
        do_action( 'biolife_before_template_part', $action_args['template_name'], $action_args['template_path'], $action_args['located'], $action_args['args'] );

        include $action_args['located'];

        do_action( 'biolife_after_template_part', $action_args['template_name'], $action_args['template_path'], $action_args['located'], $action_args['args'] );
    }
}
/**
 *
 * QUERY POSTS
 **/
if ( !function_exists( 'biolife_shortcode_posts_query' ) ) {
    function biolife_shortcode_posts_query( $atts, $post_type = 'post' )
    {
        global $post;
 ...
 * QUERY PRODUCTS
 **/
if ( !function_exists( 'biolife_shortcode_products_query' ) ) {
    function biolife_shortcode_products_query( $atts, $exclude_id = false )
    {
 ...
 **/
if ( !function_exists( 'biolife_resize_image' ) ) {
    function biolife_resize_image( $attachment_id, $width, $height, $crop = false, $use_lazy = false, $placeholder = true, $class = '' )
    {
 ...
 * GET OPTIONS
 **/
if ( !function_exists( 'biolife_file_options' ) ) {
    function biolife_file_options( $path, $name, $is_block = false )
    {
        $cache_key = '';
        $id_block  = $is_block ? 'block' : 'none-block';
        if ( biolife_get_option( 'enable_cache_option' ) ) {
            $cache_key = sanitize_key( implode( '-', array(
                'biolife-file-options',
                $name,
                $id_block,
                $path,
                BIOLIFE
...
if ( !function_exists( 'biolife_footer_preview' ) ) {
    function biolife_footer_preview( $inherit = false )
    {
        $cache_key = '';
        if ( biolife_get_option( 'enable_cache_option' ) ) {
            $cache_key = sanitize_key( implode( '-', array( 'biolife-footer-preview', BIOLIFE ) ) );
            $options   = get_transient( $cache_key );
            if ( $options ) {
                if ( $inherit == false ) {
        ...
        $footer_preview['inherit'] = array(
            'title'   => esc_html__( 'Inherit Desktop', 'biolife' ),
            'preview' => get_theme_file_uri( '/assets/images/placeholder.jpg' ),
        );
        $footer_preview['none']    = array(
            'title'   => esc_html__( 'None', 'biolife' ),
            'preview' => get_theme_file_uri( '/assets/images/placeholder.jpg' ),
        );
        $args                      = array(
...
if ( !function_exists( 'biolife_social_option' ) ) {
    function biolife_social_option( $reverse = false )
    {
        $socials     = array();
...
if ( !function_exists( 'biolife_product_options' ) ) {
    function biolife_product_options( $allow = 'Theme Option', $is_block = false )
    {
        $cache_key = '';
        $id_block  = $is_block ? 'block' : 'none-block';
        if ( biolife_get_option( 'enable_cache_option' ) ) {
            $cache_key = sanitize_key( implode( '-', array( 'biolife-product-options', $allow, $id_block, BIOLIFE ) ) );
            $options   = get_transient( $cache_key );
            if ( $options ) {
                return $options;
            }
        ...
        if ( empty( $product_options ) ) {
            $product_options['no-product'] = array(
                'title' => esc_html__( 'No Product Found', 'biolife' ),
            );
        }
...
if ( !function_exists( 'biolife_page_layout' ) ) {
    function biolife_page_layout()
    {
        if ( class_exists( 'WooCommerce' ) && is_woocommerce() ) {
            $sidebar_layout = biolife_get_option( 'sidebar_shop_layout', 'left' );
            $sidebar_name   = biolife_get_option( 'shop_used_sidebar', 'shop-widget-area' );
            if ( is_product() ) {
                $sidebar_layout = 'full';
                $sidebar_name   = '';
            ...
            if ( is_product_taxonomy() && 0 === absint( get_query_var( 'paged' ) ) ) {
                ...
                if ( $term && $term->taxonomy == 'dc_vendor_shop' ) {
                    $sidebar_name = biolife_get_option( 'shop_vendor_used_sidebar', 'vendor-widget-area' );
                }
            }
            ...
            $sidebar_name   = biolife_theme_option_meta( '_custom_page_side_options', null, 'page_sidebar', 'widget-area' );
            $sidebar_layout = biolife_theme_option_meta( '_custom_page_side_options', null, 'sidebar_page_layout', 'left' );
            if ( !is_active_sidebar( $sidebar_name ) ) {
                $sidebar_layout = 'full';
            }
        } else {
            $sidebar_layout = biolife_get_option( 'sidebar_blog_layout', 'left' );
            $sidebar_name   = biolife_get_option( 'blog_used_sidebar', 'widget-area' );
            if ( is_single() ) {
                $sidebar_layout = biolife_get_option( 'sidebar_single_layout', 'left' );
                $sidebar_name   = biolife_get_option( 'single_used_sidebar', 'widget-area' );
            }
            if ( !is_active_sidebar( $sidebar_name ) ) {
                $sidebar_layout = 'full';
            }
...
if ( !function_exists( 'biolife_breadcrumb' ) ) {
    function biolife_breadcrumb()
    {
        if ( class_exists( 'WooCommerce' ) ) {
...
if ( !function_exists( 'biolife_page_title' ) ) {
    function biolife_page_title()
    {
        if ( is_home() ) : ?>
            <?php if ( is_front_page() ) : ?>
                <h1 class="page-title entry-title">
                    <span><?php esc_html_e( 'Latest Posts', 'biolife' ); ?></span>
                </h1>
            <?php else: ?>
                <h1 class="page-title entry-title">
        ...
        <?php elseif ( is_single() ) : ?>
            <?php if ( function_exists( 'is_woocommerce' ) && is_product() ) : ?>
                <h2 class="page-title entry-title">
                    <span><?php echo esc_html__( 'Shop Product', 'biolife' ); ?></span>
                </h2>
            <?php else: ?>
                <h2 class="page-title entry-title">
                    <span><?php echo esc_html__( 'Blog Post', 'biolife' ); ?></span>
                </h2>
            <?php endif; ?>
        ...
        <?php else: ?>
            <?php if ( is_search() ) : ?>
                <h1 class="page-title entry-title">
                    <span><?php printf( esc_html__( 'Search Results for: "%s"', 'biolife' ), '<span class="search-results">' . get_search_query() . '</span>' ); ?></span>
                </h1>
            <?php else: ?>
 ...
 * );
 */
if ( !function_exists( 'biolife_add_action' ) ) {
    function biolife_add_action( $functions, $reverse = false )
    {
        if ( !empty( $functions ) ) {
...
if ( !function_exists( 'biolife_get_form_newsletter' ) ) {
    function biolife_get_form_newsletter()
    {
        $cache_key = '';
        if ( biolife_get_option( 'enable_cache_option' ) ) {
            $cache_key = sanitize_key( implode( '-', array( 'biolife-form-newsletter', BIOLIFE ) ) );
            $options   = get_transient( $cache_key );
            if ( $options ) {
                return $options;
            }
        ...
        $list_form[ $default_id ] = esc_html__( 'Default Form', 'biolife' );
...
if ( !function_exists( 'biolife_mobile_menu' ) ) {
    function biolife_mobile_menu( $menu_locations, $default = 'primary' )
    {
        if ( !empty( $menu_locations ) ) {
            ...
            $mobile_menu .= "<span class='ovic-menu-current-panel-title'>" . esc_html__( 'Main Menu', 'biolife' ) . "</span>";
